...
- [ ] 벤더로부터 EXT4 C/C++ 라이브러리 획득
- [ ] Objective-C++ 브리징 헤더 생성
- [ ] 기본 EXT4 읽기 작업 테스트
- [ ] EXT4 라이브러리 macOS 호환성 확인
- [ ] 프로젝트에 FFmpeg 라이브러리 링크
- [ ] 기본 FFmpeg Swift 래퍼 생성
- [ ] FFmpeg으로 H.264 영상 디코딩 테스트
...
- [ ] `listFiles(at:)` 메서드 구현
- [ ] `readFile(at:)` 메서드 구현
- [ ] `writeFile(data:to:)` 메서드 구현
- [ ] Swift `EXT4Bridge` 클래스 생성
- [ ] 포괄적인 오류 처리 추가
- [ ] EXT4Bridge 단위 테스트 작성
- [ ] 실제 SD 카드 하드웨어로 테스트
...
- [ ] `MetalVideoRenderer` 클래스 생성
- [ ] Metal 장치 및 커맨드 큐 초기화
- [ ] 렌더 파이프라인 디스크립터 생성
- [ ] 버텍스 셰이더 구현
- [ ] 프래그먼트 셰이더 구현
...
- [ ] `VideoPlayerView` 생성 (SwiftUI + MTKView)
- [ ] 영상 프레임 표시
- [ ] 종횡비를 올바르게 처리
- [ ] 로딩 인디케이터 추가
- [ ] 오류 표시 추가
...
- [ ] `PlayerControlsView` 생성
- [ ] 재생/일시정지 버튼 구현
- [ ] 정지 버튼 구현
- [ ] 이전/다음 파일 버튼 구현
- [ ] 타임라인 스크러버 생성
- [ ] 현재 시간 / 총 길이 표시
- [ ] 속도 제어 피커 구현 (0.5x, 1x, 2x)
- [ ] 볼륨 슬라이더 구현
- [ ] 전체 화면 버튼 추가
...
- [ ] `PlayerViewModel` 생성 (ObservableObject)
- [ ] `play()` 메서드 구현
- [ ] `pause()` 메서드 구현
- [ ] `stop()` 메서드 구현
- [ ] `seek(to:)` 메서드 구현
...
- [ ] `SyncController` 클래스 생성
- [ ] 마스터 클록 구현
- [ ] `syncPlay()` 메서드 구현
- [ ] `syncPause()` 메서드 구현
- [ ] `syncSeek(to:)` 메서드 구현
- [ ] 드리프트 모니터링 구현 (100ms마다)
- [ ] 자동 수정 구현 (드리프트 > 50ms일 때)
- [ ] 다른 프레임 레이트의 채널 처리
- [ ] 동기화 테스트 작성
...
- [ ] `MultiChannelRenderer` 클래스 생성
- [ ] 단일 패스 5-텍스처 렌더링 구현
- [ ] 그리드 레이아웃 계산기 생성 (2x3)
- [ ] 포커스 레이아웃 생성 (1개 크게 + 4개 작게)
- [ ] 수평 레이아웃 생성 (1x5)
- [ ] 누락된 채널을 우아하게 처리
- [ ] 렌더 성능 최적화
- [ ] Metal 디버거로 프로파일링
...
- [ ] `ChannelLayout` enum 생성
- [ ] `LayoutManager` 클래스 생성
- [ ] `calculateFrames(for:in:)` 메서드 구현
- [ ] 윈도우 크기 조정 처리
- [ ] 레이아웃 전환 UI 추가
- [ ] 레이아웃 기본 설정 유지
