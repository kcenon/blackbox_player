...
/// @details
/// GPS 데이터를 영상 재생 시간과 동기화하여 실시간 위치, 경로, 속도 정보를 제공합니다.
...
class GPSService: ObservableObject {
    ...
    /// @brief GPS 데이터 로드
    /// @param metadata GPS 데이터를 포함한 영상 메타데이터
    ...
    /// @brief 특정 시간의 GPS 위치 조회
    /// @param time 영상 재생 시간 (초 단위, 영상 시작부터의 경과 시간)
    /// @return 해당 시간의 GPS 좌표, 없으면 nil
    /// @details
    ...
    /// @brief 경로 세그먼트 분할
    /// @param time 현재 재생 시간 (초 단위)
    /// @return (past: 지나온 경로, future: 앞으로 갈 경로) 튜플
    /// @details
    ...
    /// @var hasData
    /// @brief GPS 데이터 존재 여부
    /// @return GPS 데이터가 있으면 true, 없으면 false
    /// @details
