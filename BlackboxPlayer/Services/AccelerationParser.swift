...
class AccelerationParser {
    ...
    ///
    /// - Parameters:
    ///   - sampleRate: 샘플링 주파수 (기본값: 10 Hz)
    ///   - format: 데이터 포맷 (기본값: Float32)
    ///
    /// ## 예시
    /// ```swift
    /// // 기본 설정 (10 Hz, Float32)
    ...
    /// - Parameters:
    ///   - data: 원시 바이너리 데이터
    ///   - baseDate: 기준 시각 (비디오 시작 시간)
    /// - Returns: AccelerationData 배열
    ...
    ///
    /// - Parameter data: 원시 데이터
    /// - Returns: 감지된 포맷 (또는 nil)
    ///
    /// ## 사용 예시
    /// ```swift
...
enum AccelDataFormat {
    ...
    /// ```
    ///
    /// **값 범위**: -3.4 × 10³⁸ ~ +3.4 × 10³⁸ (실제로는 -20G ~ +20G 사용)
    ///
    /// **정밀도**: 약 7자리 (0.00001G 단위까지 표현)
    case float32

    /// 16비트 부호있는 정수 (축당 2바이트)
    ///
    /// **메모리 레이아웃**:
    /// ```
...
enum AccelerationParserError: Error {
    /// 잘못된 데이터 포맷
    ///
    /// 예: 예상한 포맷과 실제 데이터가 맞지 않음
    case invalidFormat

    /// 데이터 부족
    ///
    /// 예: 1개 샘플 크기(6 or 12바이트)보다 작은 데이터
    case insufficientData

    /// 잘못된 값
    ///
    /// 예: G-force 값이 물리적으로 불가능한 범위 (100G 등)
    case invalidValue
}
