...
/**
 ## VideoTransformations - 영상 변환 파라미터

 GPU 셰이더에 전달할 변환 파라미터들을 담는 구조체입니다.
...
struct VideoTransformations: Codable, Equatable {
    ...
    /**
     ...
     ### 반환값:
     - **true**: 하나 이상의 변환이 활성화됨
     - **false**: 모든 값이 기본값 (변환 없음)
...
/**
 ## VideoTransformationService - 영상 변환 관리 서비스

 영상 변환 설정을 관리하고, UserDefaults에 영속적으로 저장합니다.
