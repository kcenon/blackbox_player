...
최신 블랙박스는 여러 카메라(전방, 후방, 좌, 우, 실내)에서 동시에 영상을 녹화하고 EXT4 파일 시스템으로 포맷된 SD 카드에 영상을 저장합니다. macOS는 EXT4를 네이티브로 지원하지 않기 때문에 Mac 사용자는 서드파티 솔루션 없이는 블랙박스 영상에 액세스할 수 없습니다. 이 프로젝트는 완전한 기능을 갖춘 네이티브 macOS 애플리케이션을 제공하여 이러한 격차를 해소합니다.
...
**파일 시스템:**
- **커스텀 EXT4 라이브러리**: EXT4 액세스용 C/C++ 라이브러리
- **Objective-C++**: Swift와 C++ 간의 브리지 계층
...
- **lwext4 프로젝트**: EXT4 파일 시스템 구현 참고 자료 제공
